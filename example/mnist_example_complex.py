...
# Session is used to send results before the end of a Program
...
def network(n_units, n_out, batch_size, epochs):
    ...
    while train_iter.epoch < epochs:
        ...
        """ Yield the results during a mini-batch iteration """
        Session.attach().results({
            "partial_loss": sum_loss / min(1, train_iter.current_position * batch_size),
            "partial_accuracy": sum_accuracy / min(1, train_iter.current_position * batch_size),
            "loss": global_loss,
...
def main():
    """
        This example demonstrate how one can produce results 
        before the end of an iteration.
        I.e: following the evolution between each mini-batchsize in a single epoch
    """
    # Define a curriculum for the training
    curriculum = Curriculum()
    curriculum.add(Program({"n_units": 1000, "n_outs": 10, "batch_size": 64, "epoch": 1}, [
        Target.lte("loss", 0.19), Target.gte("accuracy", 0.941), Target.lte("partial_loss", 0.19), Target.gte("partial_accuracy", 0.941)
    ]))
    curriculum.add(Program({"n_units": 1000, "n_outs": 10, "batch_size": 100, "epoch": 1}, [
        Target.lte("loss", 0.19), Target.gte("accuracy", 0.941), Target.lte("partial_loss", 0.19), Target.gte("partial_accuracy", 0.941)
    ]))
    curriculum.add(Program({"n_units": 1000, "n_outs": 10, "batch_size": 128, "epoch": 1}, [
        Target.lte("loss", 0.19), Target.gte("accuracy", 0.941), Target.lte("partial_loss", 0.19), Target.gte("partial_accuracy", 0.941)
    ]))
    ...
    visualizer = Visualizer([
        ...
        VisualizerDefinition("batch_size", "partial_loss", "Compare the partial loss for hyperparameter: batch_size, iteration #1"),
        VisualizerDefinition("batch_size", "partial_accuracy", "Compare the partial accuracy for hyperparameter: accuracy, iteration #1")
    ])
    ...
    def trainer_callback(activities):
        ...
        return {"loss": sum_loss, "accuracy": sum_accuracy, "partial_loss": 0, "partial_accuracy": 0}
